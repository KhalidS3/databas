...
INSERT INTO `kurstillfalle` VALUES (17,'SVT101','gyl',1),(18,'SVT101','gyl',3),(19,'SVT201','ala',1),(20,'SVT202','ala',2),(21,'SVT401','sna',1),(22,'KVA101','hoc',1),(23,'DJU101','hag',3),(24,'DRY101','sna',2),(25,'DRY102','sna',3),(26,'MUG101','min',4);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`user`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_lonerevision` AS select `l`.`akronym` AS `akronym`,`l`.`fornamn` AS `fornamn`,`l`.`efternamn` AS `efternamn`,`p`.`lon` AS `pre`,`l`.`lon` AS `nu`,(`l`.`lon` - `p`.`lon`) AS `diff`,round((((`l`.`lon` - `p`.`lon`) / `p`.`lon`) * 100),2) AS `proc`,if(((((`l`.`lon` - `p`.`lon`) / `p`.`lon`) * 100) >= 3),'OK','NOK') AS `mini`,`p`.`kompetens` AS `prekomp`,`l`.`kompetens` AS `nukomp`,(`l`.`kompetens` - `p`.`kompetens`) AS `diffkomp` from (`larare` `l` join `larare_pre` `p` on((`l`.`akronym` = `p`.`akronym`))) order by `l`.`akronym` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-02-05 11:22:24
