#!/usr/bin/env bash
#
# Recreate and reset the database to be as in copy table.
#
...
file="ddl_copy.sql"
echo ">>> Copy table, for backup ($file)"
mysql -uuser -ppass skolan < $file > /dev/null
...
echo ">>> Check Lönesumman = 330242, Kompetens = 19. FROM table larare: "
mysql -uuser -ppass skolan -e "SELECT SUM(lon) AS 'Lönesumma', SUM(kompetens) AS Kompetens FROM larare;"

echo ">>> Check Länesumman = 305000, Kompetens = 8. FROM table larare_pre: "
