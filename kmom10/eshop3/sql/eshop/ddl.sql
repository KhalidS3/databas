...
--
-- Seacrh for specific activity
--
DROP PROCEDURE IF EXISTS search_log;
DELIMITER ;;
CREATE PROCEDURE search_log(
    s_produkt VARCHAR(45) -- s_produkt = sök_produkt
)
BEGIN
    SELECT * FROM logg
...
ON
    produkt FOR EACH ROW
        INSERT INTO logg (beskrivning, produkt)
            VALUES ("New product added", NEW.produktid)
;;
...
ON
    produkt FOR EACH ROW
        INSERT INTO logg (beskrivning, produkt)
            VALUES ("Deleted existing product", OLD.produktid)
;;
...
ON
    produkt FOR EACH ROW
        INSERT INTO logg (beskrivning, produkt)
            VALUES ("Uppdated existing product", NEW.produktid)
;;
